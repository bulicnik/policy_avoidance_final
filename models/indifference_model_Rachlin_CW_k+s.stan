...
parameters {
  ...
  vector<lower=0.01, upper=2>[P] ss;
  vector<lower=0.01, upper=2>[P] sns;
...
model {
  ...
  for (i in 1:N) {
    real utility;
    if (w[i] == 1) {
      utility = A[i] / (1 + ks[pid[i]] * D[i]^ss[pid[i]]) - c;
    } 
    else {
      utility = A[i] / (1 + kns[pid[i]] * D[i]^sns[pid[i]]) - c;
    }
